...
import ganymedes01.etfuturum.core.utils.AxisAlignedBBExtender;
import ganymedes01.etfuturum.core.utils.GlStateManager;
import ganymedes01.etfuturum.core.utils.RenderLivingExtender;
import ganymedes01.etfuturum.entities.EntityGuardian;
import ganymedes01.etfuturum.entities.ai.Vec3i;
...
public class GuardianRenderer extends RenderLiving {
    ...
    private RenderLivingExtender RLExtender;
    private EntityGuardian theEntity;
    int field_177115_a;

    
    
	    public GuardianRenderer(ModelGuardian modelGuardian) 
	    { 
	    ...
	    {
	        if (this.RLExtender.shouldRender(livingEntity, camera, camX, camY, camZ))
	        {
	            return true;
	        }
	        else
	        {
	            if (livingEntity.hasTargetedEntity())
	            {
	                ...
	                {
	                    Vec3 vec3 = (Vec3i) this.func_177110_a(entitylivingbase, (double)entitylivingbase.height * 0.5D, 1.0F);
	                    Vec3 vec31 = (Vec3i) this.func_177110_a(livingEntity, (double)livingEntity.getEyeHeight(), 1.0F);

	                    if (camera.isBoundingBoxInFrustum(AxisAlignedBBExtender.fromBounds(vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord, vec3.yCoord, vec3.zCoord)))
	                    {
	                        return true;
	                    }
	                }
		...
		public void doRender(EntityGuardian entity, double x, double y, double z, float entityYaw, float partialTicks)
	    {
	        ...
	        if (entitylivingbase != null)
	        {
	            ...
	            //WorldRenderer worldrenderer = TessellatorExtender.getWorldRenderer();
	            //WorldRenderExtender worldrendererE = null;
	            ...
	            /*
	            ((WorldRenderExtender) worldrendererE.pos(d12, d0, d13)).tex(0.4999D, d23).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d12, 0.0D, d13)).tex(0.4999D, d22).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d14, 0.0D, d15)).tex(0.0D, d22).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d14, d0, d15)).tex(0.0D, d23).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d16, d0, d17)).tex(0.4999D, d23).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d16, 0.0D, d17)).tex(0.4999D, d22).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d18, 0.0D, d19)).tex(0.0D, d22).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d18, d0, d19)).tex(0.0D, d23).endVertex();
	            */
	            double d24 = 0.0D;
	            ...
	            /*
	            ((WorldRenderExtender) worldrendererE.pos(d4, d0, d5)).tex(0.5D, d24 + 0.5D).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d6, d0, d7)).tex(1.0D, d24 + 0.5D).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d10, d0, d11)).tex(1.0D, d24).endVertex();
	            ((WorldRenderExtender) worldrendererE.pos(d8, d0, d9)).tex(0.5D, d24).endVertex();
	            */
	            tessellator.draw();
	     ...
	     */
	    public void preRenderCallback(EntityGuardian entitylivingbaseIn, float partialTickTime)
	    {
		...
		protected ResourceLocation getEntityTexture(Entity p_110775_1_) {
			...
			return ((EntityGuardian) p_110775_1_).isElder() ? GUARDIAN_ELDER_TEXTURE : GUARDIAN_TEXTURE;
		}
