...
public class SwimNodeProcessor extends NodeProcessor {
    ...
    {
        return this.openPoint(MathHelper.floor_double(entityIn.getBoundingBox().minX), MathHelper.floor_double(entityIn.getBoundingBox().minY /*TODO + 0.5D*/), MathHelper.floor_double(entityIn.getBoundingBox().minZ));
    }
    ...
    {
        return this.openPoint(MathHelper.floor_double(x - (double)(entityIn.width / 2.0F)), MathHelper.floor_double(y /* TODO + 0.5D*/), MathHelper.floor_double(target - (double)(entityIn.width / 2.0F)));
    }
    ...
    private int func_176186_b(Entity entityIn, int x, int y, int z)
    {
        ...
        for (int i = x; i < x + this.entitySizeX; ++i)
        {
            for (int j = y; j < y + this.entitySizeY; ++j)
            {
                ...
                {
                    Block block = (this.blockaccessextender).getBlockState(blockpos$mutableblockpos.set(i, j, k)).getBlock();
